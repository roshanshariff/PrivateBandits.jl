...
\begin{multicols}{2}[]
  ...
  \item[$\vec y_t \in \Real^t$] vector of rewards up to day $t$
  \os{Can we make this notation slightly different? say, $\vec y_{\leq t}$?}
...
Rather than applying the tree-based algorithm twice, once for $\vec G_t$ and once for $\vec u_t$, we aggregate both into a single matrix an apply the tree-based algorithm to this $(d+1)\times(d+1)$-matrix. Denote $\vec A \defeq \begin{bmatrix} \vec X_n & \vec y_n \end{bmatrix} \in
