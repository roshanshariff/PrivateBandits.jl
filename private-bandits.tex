...
\phantomsection
\makeatletter\addcontentsline{toc}{chapter}{\@title}\makeatother
\maketitle
...
%We also present empirical evidence showing our analysis is in fact optimal, presented in \cref{}.
...
$d_{\rm TV}(\mathds P,\mathds Q) = \max_{\text{event
    ...
    Q}(E)}$.  %Explicit concentration bounds are given in \cref{}
...
have a private estimation of $\vec M_t$, \Cref{ass:bounded-data} must
hold.
...
\begin{theorem}[{Theorem~4.1~\citep{SheffetPrivateApproxRegression2015}}]%
...
we can infer the accurate bounds required for our regret bounds. 
Furthermore, since the Wishart noise has eigenvalues that are fairly large, we consider a post-processing of the noise matrix~--- shifting it by $c\Eye$ with
\begin{equation}
\label{eq:c}
c \defeq \tilde L^2 \paren*{\sqrt{mk} - \paren*{\sqrt{d} + \sqrt{2\ln(\nicefrac{8n}{\alpha})}}}^2-4\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})})
\end{equation}
making the bounds we require smaller than without the shift.
The
...
{\small
 ...
 \end{align*}}
 Moreover, if we are to post-process the noise by $c\Eye$ as given in Equation~\eqref{eq:c} then the following
are $(\alpha/2n)$-accurate bounds:
{\small
  \renewcommand\defeq{\!=\!}
  \begin{align*}
    \rho_{\min} \defeq 4\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})}),~~~
    \rho_{\max} \defeq 8\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})}),~~~
    \gamma \defeq \tilde L \sqrt{ \sqrt{mk} (\sqrt{d} + \sqrt{2\ln(\tfrac{2n}{\alpha})})  }.
 \end{align*}}
\end{restatable}
...
$\Wishart_{d+1}( (L^2+\tilde B^2)\Eye, k )$ and then subtracting $c\Eye$ using Equation~\eqref{eq:c}, we get a pseudo-regret bound of
\[ O\left(B \sqrt{n}\brck[\bigg]{\sigma\paren*{\log\paren[\big]{\nicefrac{1}{\alpha}}
      + d\log\paren*{n}}
      + S\tilde L \sqrt d \log(n)^{3/4}(d^{1/4} + \varepsilon^{-1/2}\log(\nicefrac 1 \delta)^{1/4})(d^{1/4}+\log(\nicefrac n \alpha)^{1/4})} \right) \] 
...
\[ O\left(\tfrac B \Delta\brck[\bigg]{\sigma\paren*{\log\paren[\big]{\nicefrac{1}{\alpha}}
      + d\log\paren*{n}}
      + S\tilde L \sqrt d \log(n)^{3/4}(d^{1/4} + \varepsilon^{-1/2}\log(\nicefrac 1 \delta)^{1/4})(d^{1/4}+\log(\nicefrac n \alpha)^{1/4})} ^2\right) \] 
...
\end{proof}
\vspace{-2.5\parskip}
\begin{proof}[Proof of \Cref{clm:LB_pulling_any_arms_many_times}]
...
\label[section]{apx_sec:more_background}

\subsection{Differential Privacy}
\label[subsection]{apx_subsec:DP}
...
\label[subsection]{apx_subsec:facts}
...
\label[subsection]{sec:regret-discussion}
...
\label[section]{apx_sec:privacy_proofs}
\newcommand{\cut}[1]{}
\cut{
Before giving the omitted proofs from the body of the paper, we wish
to have a short comparison between the regret bounds we get with the
two different techniques to introduce differential privacy. In
particular, omitting dependencies on any parameter but $n,d$ and
$\varepsilon$, we have that the pseudo-regret bound of the Wishart
noise (\cref{cor:regret_with_Wishart}) is $\tilde O(\sqrt n(d+\tfrac{\sqrt d}\varepsilon))$; and that pseudo-regret bound of the Gaussian noise (\cref{cor:regret_with_Gaussian}) is $\tilde O(\sqrt nd/\varepsilon)$. Before explaining how it came to be that the former has a better dependency on $\varepsilon$ than the latter, we first emphasize that (1) these bounds are only asymptotic (Wishart noise has far worst constants than the constants in the Gaussian bound), and that (2) this is an over simplification, as the many parameters of the problem may affect the regret bounds in favor of the Gaussian noise.
...
Admittedly, we do not know if this is merely an artifact of our
analysis, or rather truly a difference in the bound. It is possible
that slightly revising the proof of \cref{thm:linucb-regret} could
leads to different bounds, which are identical between the two
privacy-preserving techniques. To attempt to test this empirically
requires ridicolously large $d$ and $1/\varepsilon$, which is far
beyond what we could handle.  } We now provide the missing privacy
...
Gaussian. Using concentration bounds on the $\chi^2$-distribution (\cref{claim:chi2-tails}) we have that w.p. $\geq 1- \tfrac \alpha{2n}$ it holds that $\gamma=\|\vec h_t\|_{\vec H_t^{-1}}\leq \tilde L(\sqrt{d} + \sqrt{2\ln(\tfrac{2n}{\alpha})})$. %Taking a union bound over all $n$ days concludes the proof.



We now consider the case where we shift the noise matrix by some $-c\Eye$. Let as denote $\sigma_{\min} = \tilde L^2 \paren*{\sqrt{mk} - \paren*{\sqrt{d} + \sqrt{2\ln(\nicefrac{8n}{\alpha})}}}^2$ (the least eigen value of the noise matrix without any shifts). We are going to consider a shift using $c = \sigma_{\min}-4\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})})$. This makes the noise to be defined using a sample $\vec X_t \sim \Wishart_d(\tilde L^2 \Eye, mk)$ and setting $\vec H_t = \vec X_t - c\Eye$. Clearly, the bound on the smallest eigenvalue of this $\vec H_t$ is $\rho_{\min} = \sigma_{\min}-c = 4\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})})$. Moreover, the bound on the largest eigenvalue of this $\vec H_t$ is 
\begin{align*}
\rho_{\max} &= \resizebox{0.9\textwidth}{!}{$\tilde L^2 \left( \paren*{\sqrt{mk} + \paren*{\sqrt{d} + \sqrt{2\ln(\nicefrac{8n}{\alpha})}}}^2 -  \paren*{\sqrt{mk} - \paren*{\sqrt{d} + \sqrt{2\ln(\nicefrac{8n}{\alpha})}}}^2+ 4\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})}) \right)$}\cr 
&= 8\tilde L^2 \sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})})
\end{align*}
Lastly, to bound $\|\vec h_t\|_{\vec H_t^{-1}}$ we again draw back to the definition of the Wishart
distribution as the Gram matrix of samples from a multivariate
Gaussian $\Normal(\vec 0, \tilde L^2\Eye)$. Yet again, we denote this matrix of
Gaussians as $[\vec Z ; \vec z]$ where $\vec Z\in \Real^{mk\times d}$
and $\vec z \in \Real^{mk}$, and we have that $\vec H_t = \transp{\vec
  Z} \vec Z-c\Eye$ and $\vec h_t = \transp{\vec Z} \vec z$, thus $\|\vec
h_t\|_{\vec H_t^{-1}} = \sqrt{ \transp{\vec z} \vec Z (\transp{\vec  Z}
  \vec Z-c\Eye)^{-1}  \transp{\vec Z} \vec z }$. We invoke the identity
  \[ (\vec A + \vec B)^{-1} = \vec A^{-1} - \left(\Eye + \vec B^{-1}\vec A \right)^{-1} \vec A^{-1}  \] for any two invertible matrices $\vec A$ and $\vec B$, to infer that $(\transp{\vec  Z}
  \vec Z-c\Eye)^{-1} = \left( \Eye - (\Eye - \tfrac 1 c \Eye \cdot \transp{\vec  Z}  \vec Z )^{-1}\right)(\transp{\vec  Z}  \vec Z)^{-1} = \left( \Eye + (\tfrac 1 c \transp{\vec  Z}  \vec Z - \Eye )^{-1}\right)(\transp{\vec  Z}  \vec Z)^{-1}$. We have already established that the least eigenvalue of $\transp{\vec  Z}  \vec Z $ is $\sigma_{\min}$, and so the top eigenvalue of $\Eye - \tfrac 1 c \transp{\vec  Z}  \vec Z$ is at most $\tfrac{\sigma_{\min}}{c}-1$,%=\frac{4\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})})} {\sigma_{\min}-4\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})})}$, 
  thus we infer that the top eigenvalue of $\left( \Eye + (\tfrac 1 c \transp{\vec  Z}  \vec Z - \Eye)^{-1}\right)$ is at most $\tfrac {\sigma_{\min}}{\sigma_{\min}-c}$. It follows that the PSD matrix $(\transp{\vec  Z}
  \vec Z-c\Eye)^{-1}\preceq \tfrac {\sigma_{\min}}{\sigma_{\min}-c} (\transp{\vec Z} \vec Z)^{-1}$ making $\|\vec h_t\|_{\vec H_t^{-1}} \leq \sqrt{\tfrac {\sigma_{\min}}{\sigma_{\min}-c}}\cdot \sqrt{ \transp{\vec z} \vec Z (\transp{\vec  Z}  \vec Z)^{-1}  \transp{\vec Z} \vec z } \leq \sqrt{ \frac{\tilde L^2\paren*{\sqrt{mk} - \paren*{\sqrt{d} + \sqrt{2\ln(\nicefrac{8n}{\alpha})}}}^2} {4\tilde L^2\sqrt{mk}(\sqrt d + \sqrt{2\ln(\nicefrac{8n}{\alpha})})} }\tilde L(\sqrt{d} + \sqrt{2\ln(\tfrac{2n}{\alpha})}) \leq \tilde L \frac{\paren*{\sqrt{mk} - \paren*{\sqrt{d} + \sqrt{2\ln(\nicefrac{8n}{\alpha})}}}\sqrt{\sqrt{d} + \sqrt{2\ln(\tfrac{2n}{\alpha})}}} {4(mk)^{1/4}} \leq \tilde L \sqrt{ \sqrt{mk} (\sqrt{d} + \sqrt{2\ln(\tfrac{2n}{\alpha})})  }.$
\end{proof}
