...
action (or arm) and receives a corresponding reward drawn i.i.d.\@
...
\begin{restatable}[Regret of \Cref{alg:linucb}]{theorem}{ThmLinUCBRegret}%
  ...
  \begin{align*}
    \widehat R_n
    &\defeq \sum_{t=1}^n \max_{\vec x\in\Dset{t}} \innerp{\vec\theta^*}{\vec x - \vec x_t} \\
    \shortintertext{satisfies}
    \widehat R_n
    &\le \sqrt{8n}\brck*{
      \sigma\paren*{2\log\frac{2}{\alpha} + d\log\paren*{1 + \frac{nL^2}{d\rho_{\min}}}}
...
\begin{restatable}[Gap-Dependent Regret of
  \Cref{alg:linucb}]{theorem}{ThmLinUCBRegretGap}%
  \label{thm:linucb-regret}%\
  Suppose \cref{ass:x-bound,ass:meanreward-bound,ass:thetastar-bound,%
    ass:linear-reward,ass:psd-reg,ass:subgaussian} hold and the
  $\beta_t$ are as given by \cref{prop:calc-beta}.  If the optimal
  actions in every decision set $\Dset{t}$ is separated from the
  sub-optimal arms by reward gap at least $\Delta$, then with
  probability at least $1-\alpha$ the pseudo-regret of
  \cref{alg:linucb} satisfies
  \begin{align*}
    \widehat R_n
    &\le \frac{4}{\Delta} \brck*{
      \sigma\paren*{2\log\frac{2}{\alpha} + d\log\paren*{1 + \frac{nL^2}{d\rho_{\min}}}}
      + (S\sqrt{\rho_{\max}} + \gamma)\sqrt{d\log\paren*{1 + \frac{nL^2}{d\rho_{\min}}}}
      }^2
  \end{align*}
...
\begin{theorem}[\cite{SheffetPrivateApproxRegression2015}, Theorem~4.1]
  ...
  Fix positive $\varepsilon_0$ and $\delta_0$. If the $L_2$-norm of each row in the input is bouded by $\tilde L$ then adding to the inputs' $2^{\rm nd}$-moment matrix noise sampled from $\Wishart_{d+1}(\tilde L^2 \EyeNoSub, k_0)$ is $(\varepsilon_0,\delta_0)$-differentially private, provided $k_0\geq d+1 + \tfrac{28\ln(\nicefrac 4 \delta_0)}{\varepsilon_0^2}$.
  \end{theorem}
Applying this guarantee to our setting, where each count needs to preserve $(\tfrac{\varepsilon}{\sqrt{8m\ln(\nicefrac{2}{\delta})}}, \nicefrac{\delta}{2m})$-differential privacy, it suffices to sample a matrix from $\Wishart_{d+1}(\tilde{L} \EyeNoSub, k)$ with $k = \lceil d+1 + \tfrac{224m\ln(\nicefrac {8m} \delta)\ln(\nicefrac 2 \delta)}{\varepsilon^2}   \rceil$. Moreover, combining the noise of $m$ samples from the Wishart distribution is equivalent to sampling a noise matrix $N_t\sim \Wishart_{d+1}(\tilde L^2\EyeNoSub, mk)$. (Intuitively, we merely concatenate the $m$ batches of multivariate Gaussians sampled in the generation of each of the $m$ Wishart noises.) Furthermore, consider $H_t$ and $\vec h_t$ we generate from $N_t$ ($N_t$'s top-left submatrix and the right-most subcolumn resp.)  --- $H_t$ is sampled from $\Wishart_d(\tilde L^2\EyeNoSub,k)$ , and each entry of $\vec h_t$ is the dot-product of two multivariate Gaussians. Knowing their disribution, we can infer the accurate bounds required for our regret bounds.
\begin{proposition}
\label{pro:accurate_bounds_for_Wishart}
Fix any $\alpha>0$. Given that for each $t$ the regularizers $H_t, \vec h_t$ are taken by applying the tree-based algorithm with Wishart noise $\Wishart_{d+1}(\tilde L^2 \EyeNoSub, k)$, then the following $\rho_{\min},\rho_{\max}$ and $\gamma$ are $\alpha$-accurate bounds:
\begin{align*}
...
Applying Algorithm~\ref{alg:linucb} where the regularizers $H_t$ and $\vec h_t$ are derived by applying the tree-based algorithm where each node samples noise independently from $\Wishart_{d+1}( (L^2+\tilde B^2)\EyeNoSub, k )$ gives a regret bound of
\[PLUG IN THE FINAL RESULT\]
\end{corollary}
...
The result is a bound on the regret of Algorithm~\ref{alg:linucb} using the tree-based algorithm using Gaussian noise.
...
\paragraph{The Tree-Based Mechanism.} Assume for simplicity that $n=2^i$ for some positive integer $i$. Let $T$ be a complete binary tree with its leaf nodes being $l_1,...,l_n$. Each internal
...
\begin{lemma}\label{lemma:linucb-regret}
  ...
  $\abs{\innerp{\vec\theta^*}{\vec x}} \le B$ and all $H_t \succeq 0$)
  and $\beta_n \ge \max\set{\beta_0,\dotsc,\beta_{n-1},1}$.  If all
  the confidence sets $\Eset{t}$ contain $\vec\theta^*$ (i.e.,
  $\norm{V_t}{\vec\theta^* - \tilde{\vec\theta}_t} \le
  \beta_t$), then the pseudo-regret of \cref{alg:linucb} satisfies
  \begin{align*}
    \widehat{R}_n &\le \beta_n\sqrt{4n \sum_{t=1}^n \min\set{B^2,
...
\begin{proof}
  ...
  \begin{align*}
    r_t &= \innerp{\vec\theta^*}{\vec x_t^*} - \innerp{\vec\theta^*}{\vec x_t} \\
       ...
       &= \innerp{V_t^{1/2}(\bar{\vec\theta}_t - \vec\theta^*)}{V_t^{-1/2} \vec x_t}
        ...
        &\le 2\beta_n\norm{\inv{V_t}}{\vec x_t}
        &\text{since } \beta_n \ge \beta_t.
  \end{align*}
  From our assumptions that the mean absolute reward is bounded by $B$
  and $\beta_n \ge 1$, we also get that
  $r_t \le 2B \le 2B\beta_n$.  Putting these together,
  \begin{align}\label{eq:regret-bound-oneround}
    r_t &\le 2\beta_n \min\set{B, \norm{\inv{V_t}}{\vec x_t}}
  \end{align}
  ...
  \begin{align*}
    ...
    \widehat R_n &\le \sqrt{n\sum_{t=1}^n r_t^2}
                  =  2\beta_n\sqrt{n\sum_{t=1}^n\min\set{B^2,
                  \norm{\inv{V_t}}{\vec x_t}^2}}.
                  \qedhere
  \end{align*}
\end{proof}
...
\begin{lemma}[Elliptical Potential]\label{lemma:elliptical-potential}
  ...
  \begin{align*}
    ...
    &\le 2B^2 \log\frac{\det U_{n+1}}{\det U_1}
      \le 2B^2d \log\frac{\tr U_1+nL^2}{d\det^{1/d} U_1}.
  \end{align*}
\end{lemma}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    2\log\frac{\det U_{n+1}}{\det U_1}
            &= 2\sum_{t=1}^n\log(1+\norm{\inv{U_t}}{\vec x_t}^2).
  \end{align*}
  For $B \neq 1$, we apply the above result to the sequence
  $\vec x_t' \defeq \vec x_t/B$ and $U_1' = U_1/B^2$, $B^{-1}$ and $U_1$ scaled by
  $B^{-2}$:
  \begin{align*}
    \sum_{t=1}^n\min\set{B^2,\norm{\inv{U_t}}{\vec x_t}^2}
...
\begin{proof}
  ...
  \begin{align*}
    \widehat{R}_n &\le 2\beta_n\sqrt{n\sum_{t=1}^n\min\set{B^2,
                   \norm{\inv{V_t}}{\vec x_t}^2}}.
  \end{align*}
  ...
  $\norm{\inv{V_t}}{\vec x_t}^2 \le \norm{\inv{U_t}}{\vec x_t}^2$.  We
...
\todo{Finish the following gap-dependent proof.}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\le \frac{4\beta_n^2}{\Delta} \sum_{t\in B_n} \min\set{B^2, \norm{\inv{V_t}}{\vec x_t}^2}
    &\text{from \eqref{eq:regret-bound-oneround} in \cref{lemma:linucb-regret}} \\
    &\le \frac{4\beta_n^2}{\Delta} \sum_{t\in B_n} \min\set{B^2, \norm{\inv{U_t}}{\vec x_t}^2} \\
    &\le \frac{8B^2\beta_n^2d}{\Delta} \log\paren*{1 + \frac{b_n L^2}{d\rho_{\min}}}
    ...
    \widehat R_n &\le \frac{8B^2\sqrt{\beta_n}}{\Delta} d\log\frac{L^2b_t + \rho_{\min}}{\rho_{\min}}.
  \end{align*}
