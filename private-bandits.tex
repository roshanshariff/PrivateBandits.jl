...
\begin{abstract}
  ...
  standard stochastic mutli-armed bandit (MAB) problem, where a learner
...
\paragraph{MAB and the Contextual Bandit Problem.} \todo[inline]{Complete this section.}
...
\begin{multicols}{2}[]
  ...
  \item[$\vec y_t \in \Real^t$] vector of rewards up to day $t$\os{Can we make this notation slightly different? say, $\vec y_{\leq t}$?}
...
\begin{align*}
  ...
  &\defeq \min_{\hat{\vec\theta} \in \Real^d} \norm{}{\vec X_{t-1} \hat{\vec\theta}
    ...
    = \inv{(\vec G_t + \vec H_t)} \transp{\vec X_{t-1}} \vec y_{t-1}.
    &\text{where } \vec G_t \defeq \XtX{\vec X_{t-1}}
\end{align*}
...
corresponding eigenvalues of $\vec G_t$ (or $\vec V_t$). The ellipsoid
...
\begin{assumptions*}[\Cref{alg:linucb}]{%
  ...
  $y_t = \innerp{\vec\theta^*}{\vec x_t} + \eta_t$ \todo{Is it necesary? Already defined in the problem formulation. Merge with 6 perhaps?}
  \assume[ass:psd-reg] All regularizers are PSD $H_t \succeq 0$.
  \assume[ass:subgaussian] $\eta_t$ is $\sigma^2$-conditionally
  subgaussian one previous actions and rewards, i.e.:
  \begin{align*}
...
$\vec A \defeq \begin{bmatrix} \vec X_n & \vec y_n \end{bmatrix} \in
...
private approximation of $\vec M_t$. Since \cref{alg:linucb} allows for
additive perturbations $\vec H_t$ and $\vec h_t$, we can use any privacy
mechanism that uses additive noise, releasing $\vec M_t + \vec N_t$.  The
...
matrix. Under suitably chosen $\vec V$ and $k$, this noise preserves
...
Instead of ensuring the noise is symmetric by copying the upper-diagonal entries to the entries below the diagonal, we analyze here a slight variation --- where for each node we sample a matrix $\vec Z$ which all of entries are sampled iid from a Gaussian, then symmetrize $\vec Z$ using $\tfrac 1 {\sqrt 2}(\vec Z+\transp{\vec Z})$.\footnote{This increases the variance along the diagonal entries beyond the noise magnitude required to preserve privacy, but by only a constant factor of $2$.} When all entries on $\vec Z$ are sampled from $\Normal(0,\sigma_{\rm noise}^2)$, known concentration results on the top singular value of $\vec Z$ give that $\Pr[ \|\vec Z\| > \sigma_{\rm noise}(4\sqrt{d+1} + 2\ln(\nicefrac 1 \alpha)) ] < \alpha$. Note however that in each day $t$ we sum $\leq m$ such $\vec Z$ matrices (then symmetrize the noise), thus the variances of each coordinate is $m\sigma^2$. As a result, applying the union-bound, we have that w.p. $\geq 1-\alpha$ it holds that in all $n$ days our noise $\vec N_t$ has operator norm of at most $\sqrt{2m\sigma_{\rm noise}^2}(4\sqrt{d+1} + 2\ln(\nicefrac n \alpha))$. In particular, the top-left $(d\times d)$-submatrix of $\vec N_t$ has operator norm of at most $\Upsilon \defeq \sqrt{2m\sigma_{\rm noise}^2}(4\sqrt{d} + 2\ln(\nicefrac n \alpha)) = \sqrt{32} m\tilde L ^2 \ln(\nicefrac 4 \delta)(4\sqrt{d} + 2\ln(\nicefrac n \alpha)) / \varepsilon$.
...
We now give the proof of the lower bound of any private algorithm under the stadard notion of differential privacy under continual observation. First, of course, we need to define this notion. \DPDefinition
