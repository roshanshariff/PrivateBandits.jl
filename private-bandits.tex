...
\os{You seem to have a space of 2-4 lines here which you can use to discuss: Assumption 2, interplay between parameters, or the open problem in Section~\ref{sec:regret-discussion}. Your call.}
...
\os{This, where you use Assumption 2 for the first time, would be a terrific place to discuss why is it OK to set $1$ as the bound.}
...
$d$ is necessary to achieve $O(\log n)$ gap-dependent regret bounds.  Some empirical
evidence running on synthetic data with fixed reward gaps
seem to indicate approximately an $\Omega(d^{5/3})$ dependency, which shows a super-linear dependency on $d$.
