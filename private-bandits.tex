...
\item\label[assumption]{ass:x-bound} $\norm{}{\vec x} \le L$.
\item\label[assumption]{ass:thetastar-bound} $\norm{}{\vec\theta^*} \le S$.
\item\label[assumption]{ass:meanreward-bound}
  $\abs{\innerp{\vec\theta^*}{\vec x}} \le B$.
\item\label[assumption]{ass:linear-reward}
  $y_t = \innerp{\vec\theta^*}{\vec x_t} + \eta_t$
\item\label[assumption]{ass:subgaussian} $\sigma^2$-conditionally
  subgaussian reward noise $\eta_t$ given actions and previous
  rewards:
  \begin{align*}
...
\Cref{alg:linucb} relies on accurate confidence sets that nevertheless
