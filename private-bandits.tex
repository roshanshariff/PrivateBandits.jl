...
\begin{abstract}
  ...
  be a linear function of a \emph{feature vector} that encodes the
...
context, each product (action) is mapped to a feature vector; the
purchase probability is assumed to be the \emph{same} unknown linear
...
$a_t$.  One way to take advantage of the context is to assume that it
...
environment which the agent aims to learn so that she can maximize
reward.  Indeed, everything a learner needs to know about the context
is now implicitly encoded by the \emph{decision set}
$\D_t \defeq \set{\phi(c_t,a)\given a\in\A \subset \Real^d}$, and
choosing some $x_t\in\A_t$ effectively determines the action
$a_t\in\A_t$.  Thus, the contextual stochastic linear bandit framework
is composed of repeated rounds in which the learner
...
The objective is to maximizing cumulative reward, which is equivalent
to minimizing a quantity called \emph{regret}, the extra cumulative
reward a learner would have received had it always chosen the best
available action.  An effective learner may nevertheless earn only a
small reward if all the actions are poor, but it would then have low
regret.  In other words, the regret characterizes the cost of
\emph{learning} the optimal action rather than knowing it beforehand.
...
\subsubsection{Joint Differential Privacy.}%
\label{sec:joint-dp}
...
\begin{align}\label{eq:def-ucb}
  \vec x_t &\gets \argmax_{\vec x\in\D_t} \UCB_t(\vec x),
  &\text{where }
...
\begin{restatable}[Regret of \Cref{alg:linucb}]{theorem}{ThmLinUCBRegret}%
  ...
  probability at least $1-\alpha$ the pseudo-regret
  $\widehat R_n \defeq \sum_{t=1}^n \max_{\vec x\in\D_t}
  \innerp{\vec\theta^*}{\vec x - \vec x_t}$ \todo{isn't it
    $\vec x_t^*$?} of \cref{alg:linucb} satisfies
    \begin{align}
...
\begin{remark}
  ...
  the current context $c_t$ via the decision set $\D_t$.  Since
...
%   \begin{align*}
%     \widehat R_n
%     &\defeq \sum_{t=1}^n \max_{\vec x\in\D_t} \innerp{\vec x}{\vec\theta^*}
