...
performance of the (non-private) LinUCB on such a $\Delta$-gap instance.
...
\paragraph{Setting.} For each $d$ we use decision sets $\D_t$ of size
$d^2$, with an optimal arm having reward 0.75 and suboptimal actions
having rewards in $[-1, 0.25]$, so that $\Delta = 0.5$.  All the
actions are unit norm, lying on the hyper-sphere: the optimal arm is
distributed uniformly on the intersection of the sphere with the
0.75-reward hyperplane, and the suboptimal arms uniformly on the
intersection with the $<0.25$ reward half-space.  We use the same
regularizer $\rho\Eye$ on each round, with
$\rho=0.1=\rho_{\min},\rho_{\max}$ and $\gamma=0$.  The number of
rounds is $d^2$.  The noise in the rewards is drawn from the standard
gaussian $\Normal(0,1)$.  The error bars are the result of running
each experiment 160 times, and represent the range of $\pm 1$ standard
error in the mean.

We varied $d$ in the range $[10, 100]$, linearly spaced for the first
graph and logarithmically spaced for the second graph.
