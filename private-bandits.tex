...
particular regularizers into our scheme: the first is immediate, a privacy preserving mechanism that uses additive Wishart
noise~\citep{SheffetPrivateApproxRegression2015} making the noise always a PSD matrix, achieving regret of $\tilde O(\sqrt{n}(d+\nicefrac{\sqrt d}{\varepsilon}))$; then, we use the additive Gaussian
noise~\citep{DworkAnalyzeGauss2014} which we shift by $2\Upsilon\Eye$ to force the noise to be PSD (w.h.p.)\ over all days, giving a regret bound of $\tilde O(\sqrt n(d + d^{3/4}/\sqrt\varepsilon))$. The details of the two techniques are given in Section~\ref{sec:alg-dp}.
