...
\begin{abstract}
  ...
  \emph{additional} regret private algorithms incur.
 \end{abstract}
...
In the classic MAB task, in every round $t$ a learner selects an
\emph{action} $a_t$ from a fixed set $\A$ and receives a \emph{reward}
$r_t$.  In the (stationary) \emph{stochastic} MAB, the reward is noisy
with a fixed but unknown expectation $\Ex{r_t\given a_t}$ that depends
only on the selected action.
...
\begin{restatable}[Regret of \Cref{alg:linucb}]{theorem}{ThmLinUCBRegret}%
  ...
  $\widehat R_n \defeq \sum_{t=1}^n \max_{\vec x\in\Dset{t}}
